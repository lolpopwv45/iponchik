...
import { ChevronDown, Loader2, Pencil, Plus, Tag, Trash2, X } from 'lucide-react'
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {open
              ...
              : categories.length === 0
                ? 'Пока нет разделов. Откройте блок, чтобы добавить первую категорию.'
                : `${categories.length} ${categoryWord(categories.length)} — нажмите «Показать», чтобы изменить`}
          </p>
        </div>
        <button
          type="button"
          ...
          className="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50"
        >
          {open ? 'Скрыть' : 'Показать'}
          <ChevronDown
            className={cn('size-4 text-gray-500 transition-transform', open && 'rotate-180')}
            aria-hidden="true"
          />
        </button>
      </div>
...
function categoryWord(count: number) {
